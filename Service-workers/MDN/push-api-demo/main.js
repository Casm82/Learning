...
//         console.log(key);
        updateStatus(endpoint, rawKey, rawAuthSecret, 'init');
      })
      .catch(function(err) {
      ...
      navigator.serviceWorker.ready.then(function(reg) {
        reg.pushManager.subscribe({userVisibleOnly: true})
          .then(function(subscription) {
            ...
            var rawKey = subscription.getKey('p256dh');
            var rawAuthSecret = subscription.getKey('auth');
            updateStatus(endpoint, rawKey, rawAuthSecret, 'subscribe');
          })
          .catch(function(e) {
            if (Notification.permission === 'denied') {
  ...
  navigator.serviceWorker.ready.then(function(reg) {
    ...
    reg.pushManager.getSubscription().then(
      function(subscription) {
        ...
        updateStatus(endpoint, rawKey, rawAuthSecret,'unsubscribe');
...
function postSubscribeObj(statusType, name, endpoint, rawKey, rawAuthSecret) {
...
function updateStatus(endpoint, rawKey, rawAuthSecret, statusType) {
    console.log("updateStatus, endpoint: " + endpoint);
//     console.log("updateStatus, rawKey: " + rawKey);
